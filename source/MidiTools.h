...
static void sendNRPN(juce::MidiOutput* midiOutput, int channel, int parameter, int value)
{
    ...
    // Convert to zero-based channel for MIDI messages
    ...
    sequence.addEvent(juce::MidiMessage::controllerEvent(midiChannel, 99, parameterMSB));
    
    // Send CC 98 (NRPN LSB) with parameterLSB
    sequence.addEvent(juce::MidiMessage::controllerEvent(midiChannel, 98, parameterLSB));
    
    // Send CC 6 (Data Entry MSB) with valueMSB
    sequence.addEvent(juce::MidiMessage::controllerEvent(midiChannel, 6, valueMSB));

    // Optionally send CC 38 (Data Entry LSB) with valueLSB if needed
    if (valueLSB >= 0)
        sequence.addEvent(juce::MidiMessage::controllerEvent(midiChannel, 38, valueLSB));
